  ...
  return (
    <Box height={"50%"} textAlign={"center"} p="10px" border={"3px solid gray"} margin="auto" marginTop={"60px"} alignItems={"center"} width={"40%"}>
        ...
        <Heading>LOGIN / REGISTER</Heading>
         ...
         <Box m={5}>
            <Input type="text" value={num} onChange={(e)=> setNum(e.target.value)} placeholder='Enter Email ID ' />
         </Box>
         <Box m={5}>
            <Input type="text" value={num} onChange={(e)=> setNum(e.target.value)} placeholder='Enter Password' />
         </Box>
         <Box m={5} >
         ...
         <Box>
          <Link to="/SignUp" color={"blue.500"}>Sign Up</Link>
         </Box>
    </Box>
  )
}
